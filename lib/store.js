...
const { isSameDayMoscow } = require('./moscow-time');
...
// "Сегодня" считается по московскому времени (см. lib/moscow-time.js) — не
// по часовому поясу сервера, который на бесплатном хостинге может быть
// каким угодно (и не обязательно совпадать с тем, что показывают часы
// пользователя).
function isSameDay(iso1, iso2) {
  return isSameDayMoscow(iso1, iso2);
}
...
// Как sumToday, но сразу для нескольких полей за один проход по массиву —
// используется для калорий/белков/жиров/углеводов сразу, вместо того чтобы
// четыре раза фильтровать один и тот же массив foods.
function sumTodayMulti(entries, fields) {
  const today = new Date();
  const totals = {};
  fields.forEach((f) => { totals[f] = 0; });
  for (const e of entries) {
    if (!isSameDay(e.date, today)) continue;
    for (const f of fields) totals[f] += e[f] || 0;
  }
  return totals;
}

// Инструмент разработчика: сколько последних записей журнала действий хранить
// (чтобы db.json не рос бесконечно).
...
function createStore(dataFile) {
  function load() {
    ...
    try {
      ...
      if (!Array.isArray(parsed.devLog)) parsed.devLog = [];
      return parsed;
    } catch (err) {
