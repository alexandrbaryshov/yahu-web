...
// Возвращает true, если что-то реально изменилось (чтобы вызывающий код не
// сохранял состояние на диск/в Upstash впустую на каждый обычный запрос —
// подавляющее большинство вызовов приходится на дни/недели без рубежа).
function processAchievements(state) {
  ...
  const creditedSomething = creditInstantToday(state);
  return changed || creditedSomething;
}
...
function getAchievementsView(state) {
  ...
  // Один проход на каждый день недели (не два, как раньше — "сегодня" не
  // считается отдельно и ещё раз внутри цикла): последняя итерация цикла —
  // всегда сегодняшний день, из неё же берём todayCalories/todayWater.
  ...
  for (let i = 0; i < daysElapsed; i += 1) {
    ...
    const dayCalories = sumInRange(state.foods, 'calories', day, dayEnd);
    const dayWater = sumInRange(state.water, 'amount', day, dayEnd);
    if (dayCalories > 0 && dayCalories <= state.dailyTarget) caloriesGoodDays += 1;
    if (dayWater >= DAILY_TARGETS.water) waterGoodDays += 1;
    if (i === daysElapsed - 1) { todayCalories = dayCalories; todayWater = dayWater; }
  }
